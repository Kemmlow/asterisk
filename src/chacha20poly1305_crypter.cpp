...
struct ChaCha20Poly1305_Crypter::Impl {
    std::vector<std::uint8_t> key;

    explicit Impl(std::span<const std::uint8_t, KeySize> k) : key(k.begin(), k.end()) {}
};

ChaCha20Poly1305_Crypter::ChaCha20Poly1305_Crypter(std::span<const std::uint8_t, KeySize> key)
    : pimpl_(std::make_unique<Impl>(key)) {}

ChaCha20Poly1305_Crypter::~ChaCha20Poly1305_Crypter() = default;

ChaCha20Poly1305_Crypter::ChaCha20Poly1305_Crypter(ChaCha20Poly1305_Crypter&&) noexcept = default;
ChaCha20Poly1305_Crypter& ChaCha20Poly1305_Crypter::operator=(ChaCha20Poly1305_Crypter&&) noexcept = default;

[[nodiscard]] std::expected<AuthenticatedBlob, CrypterError> ChaCha20Poly1305_Crypter::encrypt(
    ...
    auto nonce = random_bytes(NonceSize);
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, static_cast<int>(NonceSize), nullptr) != 1) {
        return std::unexpected{CrypterError::EncryptionFailed};
    }

    if (EVP_EncryptInit_ex(ctx, nullptr, nullptr, pimpl_->key.data(), nonce.data()) != 1) {
        return std::unexpected{CrypterError::EncryptionFailed};
    }
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, static_cast<int>(TagSize), tag.data()) != 1) {
        return std::unexpected{CrypterError::EncryptionFailed};
    }

    ciphertext.resize(static_cast<std::size_t>(ciphertext_len));
    return AuthenticatedBlob{std::move(ciphertext), std::move(nonce), std::move(tag),
        std::vector<std::uint8_t>(associated_data.begin(), associated_data.end())};
}

[[nodiscard]] std::expected<std::vector<std::uint8_t>, CrypterError> ChaCha20Poly1305_Crypter::decrypt(
    const AuthenticatedBlob& blob
) {
    if (pimpl_->key.size() != KeySize) {
        return std::unexpected{CrypterError::InvalidKey};
    }
    if (blob.iv.size() != NonceSize) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, static_cast<int>(TagSize), const_cast<std::uint8_t*>(blob.tag.data())) != 1) {
        return std::unexpected{CrypterError::InvalidTag};
    }
